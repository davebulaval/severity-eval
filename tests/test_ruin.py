...
def test_lundberg_bound_holds():
    ...
    assert psi_mc <= lundberg_bound * 1.5, (
        f"MC ruin prob {psi_mc:.4f} > 1.5 * Lundberg bound {lundberg_bound:.4f}"
    )
...
def test_simulate_ruin_leq_lundberg_bound():
    ...
    lundberg_bound = np.exp(-R * u_star)
    assert psi <= lundberg_bound * 1.5, (
        f"ψ_MC={psi:.4f} > 1.5 * bound={lundberg_bound:.4f}"
    )
...
def test_compute_lundberg_R_rejects_shape_mismatch():
    """cost_levels and severity_profile of different length must be rejected."""
    with pytest.raises(ValueError, match="same shape"):
        compute_lundberg_R(
            CLAIM_RATE,
            cost_levels=np.array([100.0, 1000.0]),
            severity_profile=np.array([0.5, 0.3, 0.2]),
            premium_rate=PREMIUM_RATE,
        )


def test_compute_lundberg_R_rejects_negative_loading():
    """Premium below expected loss -> no positive R; must raise."""
    mu_X = (COST_LEVELS * PI).sum()
    insufficient_premium = 0.5 * CLAIM_RATE * mu_X
    with pytest.raises(ValueError, match="exceed expected loss rate"):
        compute_lundberg_R(
            CLAIM_RATE,
            COST_LEVELS,
            PI,
            premium_rate=insufficient_premium,
        )


def test_compute_lundberg_R_rejects_zero_loading():
    """Premium exactly equal to expected loss is rejected."""
    mu_X = (COST_LEVELS * PI).sum()
    with pytest.raises(ValueError, match="exceed expected loss rate"):
        compute_lundberg_R(
            CLAIM_RATE,
            COST_LEVELS,
            PI,
            premium_rate=CLAIM_RATE * mu_X,
        )


def test_compute_reserve_rejects_zero_R():
    with pytest.raises(ValueError, match="R must be positive"):
        compute_reserve(0.0)


def test_compute_reserve_rejects_invalid_ruin_target():
    R = 1e-5
    for bad in [0.0, -0.1, 1.0, 2.0]:
        with pytest.raises(ValueError, match="ruin_target"):
            compute_reserve(R, ruin_target=bad)


def test_compute_reserve_monotone_in_target():
    """Stricter target (smaller psi*) -> larger reserve."""
    R = 1e-5
    u_lax = compute_reserve(R, ruin_target=0.1)
    u_strict = compute_reserve(R, ruin_target=0.001)
    assert u_strict > u_lax


def test_compute_reserve_inversely_proportional_to_R():
    """u* = -ln(psi*) / R, so doubling R halves u*."""
    u_small_R = compute_reserve(1e-5, ruin_target=0.01)
    u_large_R = compute_reserve(2e-5, ruin_target=0.01)
    assert u_large_R == pytest.approx(u_small_R / 2, rel=1e-10)
