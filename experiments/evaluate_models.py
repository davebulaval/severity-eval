...
MODELS = {
    ...
    # --- Cohere ---
    "command-a": {"provider": "cohere", "model_id": "command-a-03-2025"},
    "command-r-plus": {"provider": "cohere", "model_id": "command-r-plus-08-2024"},
    # --- DeepSeek ---
...
def get_prompt(
    ...
    try:
        return template.format(
            question=question, context=context, options_str=options_str
        )
...
def _get_client(provider: str):
    ...
    if provider == "openai":
        ...
        _clients[provider] = (
            True  # sentinel — model created per model_id in call_google
        )
    elif provider == "cohere":
        import cohere

        _clients[provider] = cohere.ClientV2(
...
def call_openrouter(prompt: str, model_id: str) -> str:
    ...
    # Thinking models (e.g. qwen3-235b-thinking) emit <think>...</think>
    # chains before the answer. Without a larger budget the reasoning is
    # truncated and no final answer is produced.
    is_thinking = _is_thinking_model(model_id)
    response = client.chat.completions.create(
        model=model_id,
        messages=[{"role": "user", "content": prompt}],
        max_tokens=8192 if is_thinking else 512,
        temperature=0.0,
    )
    raw = (response.choices[0].message.content or "").strip()
    return _strip_think_tags(raw) if is_thinking else raw
...
def call_cohere(prompt: str, model_id: str) -> str:
    client = _get_client("cohere")
    ...
    if not response.message or not response.message.content:
        return ""
    parts = [
        c.text for c in response.message.content if getattr(c, "type", None) == "text"
    ]
    return "".join(parts).strip()


def call_deepseek(prompt: str, model_id: str) -> str:
    client = _get_client("deepseek")
    # deepseek-reasoner emits a reasoning chain via the dedicated
    # `reasoning_content` field; we only consume the final answer
    # (`content`). The 8192-token budget accommodates the chain.
    is_reasoning = "reasoner" in model_id
    kwargs = {
        "model": model_id,
        "messages": [{"role": "user", "content": prompt}],
...
def _extract_number(text: str) -> float | None:
    ...
    dollar_match = re.search(
        r"\$\s*([\d,.]+)\s*(billion|million|trillion)?", text, re.IGNORECASE
    )
    if dollar_match:
...
def _log_wandb_results(
    ...
    error_by_severity = (
        results_df[~results_df["correct"]].groupby("severity").size().to_dict()
    )
...
def _evaluate_batch_openai(
    ...
    while batch.status not in ("completed", "failed", "expired", "cancelled"):
        if time.monotonic() > poll_deadline:
            raise TimeoutError(
                f"OpenAI batch {batch.id} still {batch.status} after 72h"
            )
        ...
        log.info(
            "  Batch %s: %s (%d/%d completed)", batch.id, batch.status, completed, total
        )
...
def _evaluate_batch_anthropic(
    ...
    while message_batch.processing_status != "ended":
        if time.monotonic() > poll_deadline:
            raise TimeoutError(
                f"Anthropic batch {message_batch.id} still {message_batch.processing_status} after 72h"
            )
        time.sleep(poll_interval)
...
def _evaluate_batch_google(
    ...
    while batch_job.state.name not in completed_states:
        if time.monotonic() > poll_deadline:
            raise TimeoutError(
                f"Google batch {batch_job.name} still {batch_job.state.name} after 72h"
            )
        time.sleep(poll_interval)
...
def _evaluate_batch_xai(
    ...
    with httpx.Client(timeout=120) as http:
        ...
        while True:
            ...
            log.info(
                "  Batch %s: %d/%d done, %d pending", batch_id, success, total, pending
            )
            if pending == 0:
                break
...
def _evaluate_batch_mistral(
    ...
    while created_job.status not in terminal_states:
        if time.monotonic() > poll_deadline:
            raise TimeoutError(
                f"Mistral batch {created_job.id} still {created_job.status} after 72h"
            )
        time.sleep(poll_interval)
...
def evaluate_model(
    ...
    if provider in _BATCH_DISPATCHERS:
        try:
            ...
            log.error(
                "Batch evaluation failed for %s/%s: %s", model_name, dataset_name, e
            )
            log.error(
                "  Skipping result save — re-run to retry (no stale file created)"
            )
            return pd.DataFrame()
        return _score_and_log_results(
            results, model_name, dataset_name, output_path, n_total
        )
    ...
    def _call_single(idx_row):
        ...
        for attempt in range(max_retries):
            try:
                ...
                log.warning(
                    "Retry %d/%d for %s: %s", attempt + 1, max_retries, row["id"], e
                )
            ...
            for _, row_data in partial_df.iterrows():
                ...
                for i, (_, orig_row) in enumerate(df.iterrows()):
                    if str(orig_row["id"]) == str(row_dict.get("id")):
                        ...
                        break
            log.info(
                "  Resumed %d/%d results from %s", n_done, n_total, checkpoint_path
            )
        except Exception as e:
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {
            executor.submit(_call_single, (idx, row)): idx for idx, row in pending_items
        }
        ...
        if r is None:
            results_indexed[i] = {
                **df.iloc[i].to_dict(),
                ...
                "prediction": "",
                "correct": False,
                "score_method": "missing",
            }

    return _score_and_log_results(
        results_indexed, model_name, dataset_name, output_path, n_total
    )


def main():
    parser = argparse.ArgumentParser(
        description="Evaluate models on severity-annotated datasets"
    )
    parser.add_argument(
        "--dataset",
    ...
    parser.add_argument(
        "--limit", type=int, default=None, help="Max instances per dataset"
    )
    parser.add_argument(
        "--delay", type=float, default=2.0, help="Delay between API calls (s)"
    )
    parser.add_argument(
        "--prompt-style",
        default="original",
