...
def test_h2(
    metrics_df: pd.DataFrame, n_queries: int = 10_000, n_sim: int = 50_000
) -> dict:
...
def main():
    ...
    for domain, r in h4["per_domain"].items():
        print(
            f"  {domain}: reduction={r['mean_reduction_pct']:.1f}%, CV={r['cost_cv']:.2f}"
        )
    ...
    for domain, r in h5["per_domain"].items():
        ...
        print(
            f"  {domain}: min ρ={r['min_spearman']:.3f}, mean ρ={r['mean_spearman']:.3f} [{status}]"
        )
    print(
