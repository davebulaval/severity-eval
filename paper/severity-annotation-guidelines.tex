...
systems. The \enquote{1-10-100} rule \citep{labovitz1992,redman1998cacm}
estimates \$$1$ to prevent, \$$10$ to correct, and \$$100$ per
record per year for uncorrected errors. IBM/Ponemon's 2024 Cost of a Data Breach report measures the
average cost per intellectual-property record at \$$173$
\citep{ponemon2024} (the closest tier-equivalent to a per-instance
error cost),
consistent with this level.

\paragraph{Minor (\$1\,000).}
Corresponds to a \enquote{little~r} restatement, an immaterial correction
disclosed in subsequent filings without a Form~8-K Item~4.02 filing.
\enquote{Little~r} restatements account for $76\%$ of all
restatements as of $2020$ \citep{sec2022munter,wilmerhale2022}. The IRS accuracy-related penalty of 20\%
...
week \citep{gartner2024accounting} (industry survey of $497$
controllership professionals), with individual error remediation
...
Citibank's 2020 wire-transfer error sent \$$900$\,M to Revlon's
creditors instead of the intended \$$7.8$\,M interest payment, a
116x mis-magnitude rooted in a data-entry slip on the Flexcube UI
\citep{defontenay2021citibank}.

\paragraph{Critical (\$100\,000).}
The median FINRA disciplinary fine in 2024 was \$125\,000
\citep{eversheds2024finra,csglaw2024}. The SEC maximum penalty per single violation is
\$775\,000 \citep{harvard2016sec}. \enquote{Big~R} restatements carry an
average negative impact on net income of \$$16$M
\citep{accountingtoday2024,auditanalytics2020,acito2019restatements},
with associated stock price declines of 2--10\% \citep{gleason2008contagion}.
...
\citep{cornerstone2024scasettlement,cornerstone2024,finnerty2006sca}. JPMorgan's 2012 \enquote{London Whale} spreadsheet
error resulted in \$$6.2$\,B in losses
\citep{senate2013whale,prosperspark2023}.
...
\texttt{dollar\_large}
  & \$X in M/B
  & Answer contains \$-amount AND (answer mentions \enquote{million}/\enquote{billion} OR
    value $\geq 50$ AND question mentions \enquote{million}/\enquote{billion} OR value
    $> 500$ without scale) \\
\texttt{dollar\_small}
  & \$X per-share
  & Answer contains \$-amount with value $< 50$ (per-share, dividend,
    small absolute) \\
\texttt{percentage}
  & X\%
  & Answer contains \enquote{\%} or \enquote{percent} \\
\texttt{ratio}
  & Plain number
  & Answer matches \texttt{-?[\textbackslash d,.]+x?} \\
\texttt{yes\_no}
  & Yes/No
  & Answer starts with \enquote{yes} or \enquote{no} \\
\texttt{text}
  & Free-form
  & All other answers \\
\bottomrule
...
(\enquote{Is the company capital-intensive?}) has the least direct
...
\centering
\footnotesize
\begin{tabular}{@{}lp{4.5cm}p{8cm}@{}}
\toprule
Code & SEC/GAAP category & Keywords (in question) \\
\midrule
\texttt{core}
  & Income statement, balance sheet line items
  & revenue, net income, net earnings, net loss, total assets, total
    liabilities, total equity, gross profit, COGS, cost of sales \\
\texttt{operational}
  & Cash flow, working capital, capital structure
  & EBITDA, operating income, cash flow, capex, capital expenditure,
    working capital, debt, inventory, accounts receivable/payable,
    pension, restructuring, derivatives \\
\texttt{ratio}
  & Derived metrics (ratios, margins, per-share)
  & ratio, margin, growth rate, return on, ROE, ROA, EPS, per share,
    yield, turnover, leverage, liquidity, P/E \\
\texttt{descriptive}
  & Qualitative, non-numerical
  & products and services, key agenda, describe, list, which securities \\
\bottomrule
\end{tabular}
...
error on these items directly affects the \enquote{top line} or \enquote{bottom line}
...
\begin{tabular}{@{}lcccc@{}}
\toprule
& \texttt{core} & \texttt{operat.} & \texttt{ratio} & \texttt{descr.} \\
\midrule
\texttt{dollar\_large} & \cellcolor{sevCrit!22}critical & \cellcolor{sevMaj!22}major & \cellcolor{sevMaj!22}major & \cellcolor{sevMaj!22}major \\
\texttt{dollar\_small} & \cellcolor{sevMin!22}minor    & \cellcolor{sevMin!22}minor & \cellcolor{sevMin!22}minor & \cellcolor{sevNeg!18}neglig. \\
\texttt{percentage}    & \cellcolor{sevMin!22}minor    & \cellcolor{sevMin!22}minor & \cellcolor{sevMin!22}minor & \cellcolor{sevMin!22}minor \\
\texttt{ratio}         & \cellcolor{sevMin!22}minor    & \cellcolor{sevMin!22}minor & \cellcolor{sevMin!22}minor & \cellcolor{sevMin!22}minor \\
\texttt{yes\_no}       & \cellcolor{sevMaj!22}major    & \cellcolor{sevMaj!22}major & \cellcolor{sevMin!22}minor & \cellcolor{sevMin!22}minor \\
\texttt{text}          & \cellcolor{sevMaj!22}major    & \cellcolor{sevMin!22}minor & \cellcolor{sevMin!22}minor & \cellcolor{sevNeg!18}neglig. \\
\bottomrule
\end{tabular}%
}%
\caption{Severity matrix: answer type (rows) $\times$ metric type
(columns). Wong colorblind-safe palette: vermillion (critical),
orange (major), sky blue (minor), blue (negligible).}
...
\item \textbf{Core + dollar\_large = critical.}
    ...
    scrutiny (SAB~99 \enquote{Big~R} threshold).
\item \textbf{Ratios and percentages are always minor.}
    ...
    distinction between \enquote{core revenue} and \enquote{revenue growth rate \%}
    matters more than the underlying metric category.
\item \textbf{Yes/No on core or operational topics = major.}
    Binary assessments about financial health (e.g., \enquote{Does the company
...
using the rubric in \autoref{tab:severity-scale} and a one-shot
...
\autoref{tab:llm-agreement} reports the inter-annotator agreement
...
\autoref{tab:evidence-summary} compiles the regulatory and market
...
\midrule
\multicolumn{3}{@{}l}{\itshape Negligible (\$100)} \\
1-10-100 Rule          & per-record/yr data error      & \$100 \\
IBM/Ponemon 2024       & per IP record                 & \$173 \\
\addlinespace
\multicolumn{3}{@{}l}{\itshape Minor (\$1k)} \\
IRS accuracy penalty   & 20\% of \$5k understatement   & \$1\,000 \\
\addlinespace
\multicolumn{3}{@{}l}{\itshape Major (\$10k)} \\
SOX \S906 individual   & negligent certification       & \$10\,000 \\
SEC late filing        & per-firm penalty              & \$10\,000--97\,000 \\
\addlinespace
\multicolumn{3}{@{}l}{\itshape Critical (\$100k)} \\
FINRA median fine      & disciplinary action 2024      & \$125\,000 \\
SEC max per violation  & single act / omission         & \$775\,000 \\
ORX banking            & mean loss event 2023          & EUR\,232\,000 \\
\addlinespace
\multicolumn{3}{@{}l}{\itshape Beyond critical (\$M--\$B systemic exposure)} \\
SOX \S906 willful      & willful certification         & \$5M \\
\enquote{Big~R} restatement      & avg.\ net income impact       & --\$16M \\
Class action median    & securities settlement 2024    & \$14M \\
Class action mean      & securities settlement 2024    & \$42.4M \\
Citibank-Revlon 2020   & wire-transfer error           & \$900M \\
JPMorgan 2012          & \enquote{London Whale} spreadsheet      & \$6.2B \\
GAO 1997--2002         & aggregate cap.\ loss          & \$100B+ \\
Gartner                & avg.\ annual cost, poor data  & \$12.9--15M/firm \\
IBM                    & U.S.\ annual cost, poor data  & \$3.1T \\
\bottomrule
\end{tabular}
\caption{Empirical evidence grouped by the severity tier it
calibrates. Amounts beyond the \$100k critical threshold illustrate
that the per-query scale captures the \emph{unit cost} of one
error; systemic exposure is modelled through the compound loss
$S = \sum X_i$.}
...
Major      & 44 & 29.3\% & Operational \$M/B, financial health \\
Critical   &  5 &  3.3\% & Core financials in \$M/B \\
\bottomrule
\end{tabular}
