...
\citet{beauchemin2026position} recently argued that actuarial
...
We report an empirical study applying this framework to up to 23
language models---frontier APIs together with locally deployed
open-weight models in the 8B--235B parameter range---across 11
benchmarks in four high-stakes domains (finance, medicine, law, and
insurance). Each benchmark instance carries a severity annotation
grounded in domain-specific regulatory and economic evidence. Using
the compound loss model $S = \sum_{i=1}^{N} X_i$ with
$N \sim \mathrm{Bin}(n, p)$ and $X_i$ drawn from a categorical
severity distribution, we estimate the expected aggregate loss, the
tail risk measures $\VaR_{0.95}$ and $\TVaR_{0.95}$, and the Lundberg
reserve for each model--dataset pair.

We organize our analysis around five research questions about the
relationship between accuracy and economic risk: whether model
rankings by expected loss differ from rankings by accuracy, how
inter-model variance in $\ES$ decomposes into a severity and a
frequency component, whether concrete ranking inversions occur,
whether human-in-the-loop routing modelled as excess-of-loss
reinsurance yields meaningful cost reductions, and how sensitive
the resulting rankings are to perturbations of the cost vector. We release \seveval{}, an open-source Python library
compatible with the HuggingFace \texttt{evaluate} ecosystem, along
with severity annotations for all 11 benchmarks.
\end{abstract}
...
This paper provides empirical grounding for the framework. We
evaluate up to 23 language models on 11 benchmarks spanning finance,
medicine, law, and insurance. For each benchmark, we define a
four-level severity taxonomy calibrated against publicly available
regulatory penalties, litigation data, and operational loss
references for that domain. Each test instance is annotated with a
severity level---either derived from structured metadata in the
dataset (e.g., clinical calculator categories in MedCalc-Bench,
clause types in CUAD) or via a rule-based system, with the
rule-based subset spot-checked against an independent LLM
annotator. We then apply the actuarial framework to estimate, for
each model--dataset pair, the expected aggregate loss $\ES$, the
value-at-risk $\VaR_{0.95}(S)$, the tail value-at-risk
$\TVaR_{0.95}(S)$, and the Lundberg reserve.

Our contributions are:
\begin{enumerate}[leftmargin=*]
  \item \textbf{Multi-model, multi-domain evaluation.} We report
    accuracy and actuarial risk measures for the
    model$\times$benchmark pairs we evaluate. Where data permit we
    examine whether the divergence between accuracy and expected
    loss observed by \citet{beauchemin2026position} on a single
    domain generalizes to other domains and scales.
  \item \textbf{Severity-annotated benchmarks.} We release severity
    annotations for the benchmarks we evaluate, along with the
    annotation rules, calibration evidence, and validation protocol
    (\S\ref{sec:severity-annotation}, Appendix~\ref{sec:app-calibration}).
  \item \textbf{Open-source library.} We release \seveval{}, a Python
    library compatible with the HuggingFace \texttt{evaluate}
    ecosystem, implementing the compound loss simulation, the risk
    measures, ruin-theoretic reserves, and the routing analysis
    (\S\ref{sec:library}).
  \item \textbf{Reusable evaluation protocol.} We formalize five
    research questions and the statistical summaries that future
    severity-aware evaluations can report (\S\ref{sec:protocol}).
\end{enumerate}
...
An AI system produces $n$ outputs per evaluation period, each correct
(probability $1-p$) or erroneous (probability $p$). Each error falls
into one of $K$ severity categories with costs $c_1 < \cdots < c_K$
and probabilities $\bpi = (\pi_1, \ldots, \pi_K)$:
\begin{align}
...
\midrule
\multirow{3}{*}{Medical}
  & MedCalc-Bench & 1,047 & Clin.\ calc.\    & Calculator category          & \citet{kazi2024medcalc} \\
  & MedQA (USMLE) & 1,273 & MCQ              & Clinical topic               & \citet{jin2021medqa} \\
  & HEAD-QA       & 2,742 & MCQ              & Topic domain                 & \citet{vilares2019headqa} \\
\midrule
\multirow{3}{*}{Legal}
  & CUAD          & 4,182 & Clause extr.\    & Clause category              & \citet{hendrycks2021cuad} \\
  & MAUD          & 1,396 & Merger QA        & Deal point type              & \citet{wang2023maud} \\
  & ContractNLI   & 607   & NLI              & NDA clause type              & \citet{koreeda2021contractnli} \\
\midrule
\multirow{2}{*}{Insurance}
  & RAG Insurance & 82    & QA (FR)          & Expert annotation            & \citet{beauchemin2024quebec} \\
  & JudgeBERT     & 807   & Simplification   & Meaning preserv.\            & \citet{beauchemin2025thesis} \\
\bottomrule
\end{tabular}
...
\paragraph{RAG Insurance} \citep{beauchemin2024quebec} contains 82
...
in \citet{beauchemin2026position}. \textbf{JudgeBERT}
...
We evaluate 23 language models spanning eight provider families and
four scale tiers. Table~\ref{tab:models} lists the models. Provider
budgets and pricing led us to select recent flagship and mid-tier
models from each family rather than running every available
checkpoint.
...
Qwen3-235B (think./std.) & Alibaba (OpenRouter) & Reasoning/General \\
\midrule
\multicolumn{3}{l}{\textit{Open-weight local models (4-bit NF4 quantization)}} \\
...
respective provider endpoints (with the official batch APIs used
where available for cost reduction). Local models are deployed with
4-bit NF4 quantization via unsloth/bitsandbytes on a single GPU.}
\label{tab:models}
\end{table}

API models are queried through official endpoints, with batch APIs
used for OpenAI, Anthropic, Google, xAI and Mistral; Cohere,
DeepSeek and OpenRouter (Qwen3-235B) calls use a parallel synchronous
fallback. Local open-weight models are deployed with 4-bit NF4
quantization \citep{dettmers2023qlora} using unsloth
\citep{unsloth2024} on single NVIDIA GPUs. We use adaptive inference
...
dataset's published evaluation protocol (Appendix~\ref{sec:app-prompts}),
and \emph{standard}, a uniform zero-shot format modeled after HELM
\citep{liang2023helm}. The original prompts serve as the primary
evaluation; the standard prompts serve as an ablation to test whether
model rankings are robust to prompt variation.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item The full distribution of $S$ via $10^5$ Monte Carlo
    ...
    (Equations~\eqref{eq:frequency}--\eqref{eq:expected_loss}), assuming
...
We organize the analysis around five research questions that probe the
relationship between accuracy and the actuarial risk measures defined
in Section~\ref{sec:framework}. For each question, we report a
quantitative criterion that we use to summarize the empirical signal.
We do not commit ex ante to thresholds for ``significance''; the
criteria below are reporting conventions, not pass/fail tests.

\begin{description}[leftmargin=0pt,labelindent=0pt]
  \item[RQ1 (Ranking divergence).] How closely aligned are model
    rankings by accuracy and by $\ES$ within a domain? We report
    Kendall $\tau$ between the two rank orders, together with its
    $p$-value.
  \item[RQ2 (Severity vs.\ frequency).] How much of the inter-model
    variance in $\ES$ is attributable to the severity profile $\bpi$
    versus the error rate $p$? We report a variance decomposition that
    holds one factor at the domain mean while letting the other vary.
  \item[RQ3 (Ranking inversions).] Are there concrete pairs $(A, B)$
    such that $\mathrm{acc}(A) > \mathrm{acc}(B)$ but
    $\ES(A) > \ES(B)$? We report the count and dollar magnitude of
    such inversions per dataset.
  \item[RQ4 (Routing benefit).] Does the cost reduction from HITL
    routing covary with the tail heaviness of the domain's cost
    distribution? We report the per-domain mean cost reduction and
    the Spearman correlation with the coefficient of variation of
    $\mathbf{c}$.
  \item[RQ5 (Robustness).] How sensitive are model rankings to cost
    calibration? We perturb each $c_k$ by $\pm 20\%$ and report the
    minimum Spearman $\rho$ between the base and perturbed rankings.
\end{description}
...
the actuarial evaluation pipeline. The library is designed around
three principles: (1)~a HuggingFace-style \texttt{evaluate.Metric}
subclass (\texttt{CompoundLossMetric}) so the risk measures can be
plugged into an existing evaluation pipeline alongside accuracy;
(2)~minimal core dependencies (NumPy, SciPy), with pandas and
matplotlib loaded lazily only when DataFrame export or plotting is
requested; and (3)~domain-agnostic design with built-in taxonomies
for finance, medicine, law and insurance, plus a registration API
for custom domains. The actuarial pipeline is split across six
modules---compound loss simulation, risk measures, ruin theory,
routing analysis, severity taxonomies, and sensitivity
analysis---documented with usage examples in
Appendix~\ref{sec:app-library}.
...
organised around the five research questions of
...
provided in Appendix~\ref{sec:app-full-results}; we highlight here the
aggregate signals. The numbers below correspond to the snapshot of
runs released with the code; the same pipeline can be re-executed at
the full scale of Section~\ref{sec:experiments} via the
\texttt{run\_full\_pipeline.sh} script described in
Appendix~\ref{sec:app-library}.

\paragraph{Sample size caveat.}
At the time of writing, only the finance domain (FinanceBench) had
sufficient model coverage to evaluate ranking-based questions (RQ1,
RQ3, RQ5) with more than three models. The remaining domains carry
single-model coverage on most datasets, so we report the
single-domain numbers as preliminary and defer cross-domain
comparison to future work using the released pipeline.

\subsection{RQ1: Ranking divergence}
\label{sec:rq1}

Across $12$ frontier and open-weight models evaluated on
FinanceBench, the rankings induced by accuracy and by $\ES$ are
almost monotonically aligned: Kendall $\tau = 0.96$ ($p = 5\times
10^{-4}$). In this preliminary snapshot, accuracy is a faithful
proxy for $\ES$ at the model level. Whether this alignment
persists in domains with heavier-tailed cost vectors (medical,
insurance) is the central open question that the full evaluation
addresses.

\subsection{RQ2: Severity vs.\ frequency variance}
\label{sec:rq2}

Decomposing inter-model variance in $\ES$ into a severity-only and
a frequency-only contribution (Section~\ref{sec:protocol}) yields,
for finance, $\mathrm{Var}_{\bpi} = 1.28\times 10^{14}$ and
$\mathrm{Var}_p = 8.24 \times 10^{13}$, so the severity share is
$\mathrm{Var}_{\bpi} / (\mathrm{Var}_{\bpi} + \mathrm{Var}_p) =
0.61$. The severity profile $\bpi$ accounts for a larger share of
inter-model risk variance than the error rate $p$ in this domain.
This decomposition is sensitive to the cost vector and to the model
mix; we report it as descriptive rather than confirmatory.

\subsection{RQ3: Ranking inversions}
\label{sec:rq3}

In the FinanceBench subset, where $\tau$ is close to one, we
observe $0$ strict inversions of the form $\mathrm{acc}(A) >
\mathrm{acc}(B)$ \emph{and} $\ES(A) > \ES(B)$. This is consistent
with the high $\tau$ above. Whether inversions emerge in other
domains, or under different prompt styles, remains open.

\subsection{RQ4: Routing benefit}
\label{sec:rq4}

Using a retention threshold of $d = c_3$ (route ``major'' and
``critical'' errors) and human-review cost $h = \$50$, the
estimated mean cost reduction (Table~\ref{tab:h4}) is $85.1\%$ in
finance, $99.9\%$ in insurance (single model), $66.6\%$ in medical
and $50.0\%$ in legal. The Spearman correlation between the
coefficient of variation of $\mathbf{c}$ and the mean reduction is
upward in the snapshot but cannot be tested rigorously with only
four domains and uneven within-domain coverage.

\subsection{RQ5: Robustness to cost calibration}
\label{sec:rq5}

Perturbing each $c_k$ by $\pm 20\%$ (one coordinate at a time,
$\pm$ directions) leaves the FinanceBench $\ES$ ranking unchanged
in our snapshot: the minimum Spearman correlation between the base
and perturbed rankings is $1.00$. With only one domain reaching
the three-model threshold, the result is preliminary, but it
suggests that moderate uncertainty in $\mathbf{c}$ does not
overturn rankings dominated by large differences in $\bpi$ and $p$.
...
\subsection{What the actuarial framework adds}

The compound loss model produces three families of quantities that
accuracy alone does not provide. The expected loss $\ES$ summarizes
the per-period economic exposure under a given $\mathbf{c}$; the tail
measures $\VaR_{0.95}$ and $\TVaR_{0.95}$ characterise the loss in
adverse scenarios; the Lundberg reserve $u^\ast$ translates a
target ruin tolerance into a dollar budget. These outputs are
...
and $\ES$ rankings agree (as in our finance snapshot), severity
profiles can account for a majority of inter-model variance in
$\ES$. The mechanism is intuitive: $\ES = n p \mu_X$, and $\mu_X$
spans three orders of magnitude across severity levels in all four
of our taxonomies. Two systems with the same $p$ but different
$\bpi$ produce $\ES$ values that can differ by an order of
magnitude. The framework adds value primarily in the
heterogeneous-severity regime; when an entire benchmark collapses
to one severity level, $\ES$ is proportional to $p$ and accuracy is
...
conditioned on an undetected error. The robustness analysis (RQ5)
...
domains and 23 models in Section~\ref{sec:experiments} is the natural
...
\paragraph{Cost calibration subjectivity.}
While our cost vectors are grounded in regulatory and market evidence,
the mapping from severity labels to dollar amounts is inherently
approximate. Different organizations, even within the same domain,
may face different cost structures depending on their size, regulatory
exposure, and risk tolerance. We include a robustness analysis (RQ5)
to test whether rankings are stable under moderate perturbation, but
practitioners should recalibrate cost levels to their specific context.

\paragraph{Binary correctness assumption.}
Our framework classifies each model output as either correct or
erroneous, discarding the spectrum of partial correctness that
characterizes many real-world outputs. A financial answer that reports
revenue as \$14.0B instead of \$14.2B is ``wrong'' but far less
consequential than one reporting \$11.6B. Extending the framework to
continuous severity scoring is a direction for future work.

\paragraph{Severity annotation coverage.}
For datasets where severity is derived from structured metadata (e.g.,
calculator type in MedCalc-Bench, clause type in CUAD), the annotation
is deterministic and reproducible. For datasets requiring rule-based
mapping (FinanceBench, MedQA), the rules introduce a layer of
judgment. Our inter-annotator validation (Cohen's $\kappa = 0.46$,
adjacent agreement 94\%) suggests moderate agreement, with most
disagreements confined to adjacent severity levels.

\paragraph{Dataset scope.}
We evaluate on English-language benchmarks (except RAG Insurance, in
French) across four domains. The framework is domain-agnostic, but the
specific cost calibrations and severity taxonomies may not transfer
directly to other domains (e.g., code generation, content moderation)
without domain-expert input.

\paragraph{Static evaluation.}
Our evaluation captures model performance at a single point in time.
Model capabilities evolve through updates and fine-tuning; the
severity profile of a model may shift between evaluation and
deployment. Continuous monitoring using the \seveval{} library can
...
writing), evaluates open-weight models locally with $4$-bit NF4
quantisation via unsloth and bitsandbytes
\citep{dettmers2023qlora,unsloth2024}, runs the analysis
