...
aggregate signals. At the time of writing, only the finance domain
(FinanceBench) had sufficient model coverage to evaluate
ranking-based questions with more than three models; the remaining
domains carry single-model coverage on most datasets, so we report
the single-domain numbers as preliminary and defer cross-domain
...
10^{-4}$), and we observe $0$ strict inversions of the form
$\mathrm{acc}(A) > \mathrm{acc}(B)$ \emph{and} $\ES(A) > \ES(B)$ in
this subset. In this preliminary snapshot, accuracy is a faithful
proxy for $\ES$ at the model level. Whether this alignment persists
in domains with heavier-tailed cost vectors, and whether inversions
emerge there, is the central open question that the full evaluation
addresses (Figure~\ref{fig:ranking} shows the per-domain
$\tau$ once cross-domain coverage is complete).
...
This decomposition is sensitive to the cost vector and the model
mix; we report it as descriptive rather than confirmatory and
include per-domain profiles in Figure~\ref{fig:profiles}.
...
insurance (single model), $66.6\%$ in medical and $50.0\%$ in legal
(Figure~\ref{fig:routing}). The Spearman correlation between the
