...
hallucination is penalised like a rounded decimal. We operationalise
the actuarial frequency-severity framework of
\citet{beauchemin2026position} by annotating eight benchmarks
(finance, medicine, law, insurance) with a per-instance cost of error
and evaluating up to 29 language models on the resulting compound
loss $S = \sum_{i=1}^{N} X_i$. For each model-dataset pair we report
expected loss, $\VaR_{0.95}$, $\TVaR_{0.95}$, the Lundberg reserve,
and a human-in-the-loop routing analysis. We release \seveval{}, a
HuggingFace-compatible Python library, alongside the annotations.
\end{abstract}
...
providers) is listed in Appendix~\ref{sec:app-models}.
...
estimated mean cost reduction is $85.1\%$ in finance, $99.9\%$ in
insurance (single model), $66.6\%$ in medical and $50.0\%$ in legal. The Spearman correlation between the
