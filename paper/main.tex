...
with severity annotations for the eight benchmarks evaluated here.
\end{abstract}
...
evaluate up to 23 language models on eight benchmarks spanning
finance, medicine, law, and insurance. For each benchmark, we
define a four-level severity taxonomy calibrated against publicly
available regulatory penalties, litigation data, and operational
loss references for that domain. Each test instance is annotated
with a severity level derived from a structured categorical field
in the source dataset (clause type in CUAD, deal-point category in
MAUD, fixed hypothesis in ContractNLI, academic category in
HEAD-QA) or, for the three finance benchmarks, from a matrix over
answer/program/metric type axes; the latter subset is spot-checked
against an independent LLM annotator (Section~\ref{sec:limitations}). We then apply the actuarial framework to estimate, for
...
\paragraph{Severity annotation provenance.}
Our retained benchmarks split into two annotation regimes, and we
report results for both groups separately so the reader can see how
much the weaker regime moves headline numbers.

\textbf{Domain-expert-derived (CUAD, MAUD, ContractNLI, HEAD-QA,
JudgeBERT).} Each instance carries a structured categorical field
(clause type, MAUD deal-point category, fixed ContractNLI hypothesis,
HEAD-QA academic category, JudgeBERT simplified-clause category) that
an expert taxonomy maps deterministically to a severity level. The
mapping is reproducible, traceable, and does not re-interpret the raw
question text. These five datasets account for the large majority of
the annotated rows in our evaluation.

\textbf{Matrix-heuristic (FinanceBench, FinQA, TAT-QA).} Severity is
inferred from a cross-tabulation over structured axes of the source
dataset (answer/program type $\times$ metric type), not from raw
keyword matching. The matrices encode domain priors but were not
validated against expert annotation on every instance. To bound the
risk we ran inter-annotator validation on a stratified 100-instance
sample per finance dataset (Cohen's $\kappa = 0.46$, adjacent
agreement 94\%): agreement is moderate, with most disagreements
confined to adjacent severity levels rather than across the
negligible-vs-critical gap.

We initially considered three additional benchmarks---MedQA,
MedCalc-Bench, and the RAG Insurance corpus---whose severity labels
came from keyword matching over the raw question text or calculator
name. After the same inter-annotator audit failed to defend their
critical-vs-major boundary, we removed them from the evaluation
rather than report numbers we could not stand behind. We retain the
loaders in the released code base for replication of their published
benchmarks under user-supplied severity rules.

\paragraph{Dataset scope.}
We evaluate on English-language benchmarks across four domains
(finance, medicine, law, insurance). The framework is domain-agnostic, but the
