...
See \autoref{sec:severity-scale} for full calibration.

\textbf{Key sources:} SEC SAB~99 materiality framework; FINRA median
fine \$125k (2024); SOX~\S906 individual penalty \$10k; \enquote{Big~R}
restatement average net income impact $-$\$$16$\,M; IBM/Ponemon
cost per intellectual-property record \$$173$.
...
\begin{tabular}{@{}llrl@{}}
\toprule
Domain & Level & $c_k$ & Calibration example \\
\midrule
\multirow{4}{*}{Medical}
...
\bottomrule
\end{tabular}
\caption{Unified severity scales for the four domains beyond the
core finance/medical/legal/insurance benchmarks evaluated here
(Medical detail, Legal detail, Code/Security, Moderation). Each cost
vector spans three orders of magnitude. The Finance scale
(\autoref{tab:severity-scale}) follows the same structure.}
\label{tab:domain-scales}
\end{table*}
...
\item \textbf{Diagnostic error cost:} Estimated \$100B/year in the U.S.
    ...
    diagnosis: \$36\,000--\$85\,000 \citep{newmantoker2024burden}.
...
\paragraph{Annotation rule for HEAD-QA.}
HEAD-QA tags each question with one of six academic categories.
The error-cost mapping is:
\begin{itemize}
\item Pharmacology $\to$ \textit{critical}
\item Medicine, Nursing $\to$ \textit{major}
\item Psychology $\to$ \textit{minor}
\item Biology, Chemistry $\to$ \textit{negligible}
\end{itemize}
...
\item \textbf{IBM/Ponemon 2024:} Average cost of a data breach:
    ...
    Average cost per intellectual-property record: \$$173$. A critical vulnerability
...
\item \textbf{Advertiser boycotts:} The 2020 \enquote{Stop Hate for Profit}
