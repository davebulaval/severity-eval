...
def sensitivity_analysis(
    ...
    if not (0.0 < perturbation < 1.0):
        raise ValueError(
            f"perturbation must be in (0, 1) to keep all costs positive, "
            f"got {perturbation}"
        )
    if set(error_rates) != set(severity_profiles):
        raise ValueError(
            "error_rates and severity_profiles must have the same model keys "
            f"(got {sorted(error_rates)} vs {sorted(severity_profiles)})"
        )

    cost_levels = np.asarray(cost_levels, dtype=np.float64)
    # Validate without mutating the caller's dict.
    severity_profiles = {
        name: validate_severity_profile(pi) for name, pi in severity_profiles.items()
    }
