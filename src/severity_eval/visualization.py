...
def plot_aggregate_loss_distribution(
    ...
    ax.axvline(
        ...
        linestyle="--",
        linewidth=2,
        label=f"E[S] = {expected_loss:,.0f}",
    )
    ax.axvline(
        var, color="#DD8452", linestyle="-.", linewidth=2, label=f"VaR = {var:,.0f}"
    )
    ax.axvline(
        tvar, color="#55A868", linestyle=":", linewidth=2, label=f"TVaR = {tvar:,.0f}"
    )
...
def plot_severity_profiles(
    ...
    for j, label in enumerate(labels):
        ...
        ax.bar(
            x, values, width, bottom=bottom, label=label, color=colors[j % len(colors)]
        )
        bottom += values
...
def plot_routing_impact(
    ...
    ax.bar(
        ...
        expected_loss,
        width,
        label="E[S] (no routing)",
        color="#C44E52",
        alpha=0.8,
    )
    ax.bar(
        x + width / 2,
        expected_cost_routed,
        width,
        label="E[C] (with routing)",
        color="#4C72B0",
        alpha=0.8,
    )
