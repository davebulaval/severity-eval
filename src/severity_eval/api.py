...
class SeverityReport:
    ...
    def summary(self) -> str:
        ...
        for i, (label, cost) in enumerate(
            zip(self.labels, self.cost_levels, strict=True)
        ):
        ...
        if self.routing is not None:
            ...
            w(
                f"  Retained (AI) — {len(r.retained_cost_levels)} levels, mu_X_ret=${r.mu_X_retained:,.0f}\n"
            )
            for c_k, pi_k in zip(
                r.retained_cost_levels, r.retained_severity_profile, strict=True
            ):
                w(f"    c_k=${c_k:>10,.0f}   pi_k={pi_k:>6.1%}\n")

            w(
                f"  Routed (human) — {len(r.routed_cost_levels)} levels, h=${r.human_review_cost:,}/query\n"
            )
            for c_k, pi_k in zip(
                r.routed_cost_levels, r.routed_severity_profile, strict=True
            ):
                w(f"    c_k=${c_k:>10,.0f}   pi_k={pi_k:>6.1%}\n")
    ...
    def to_latex(self) -> str:
        ...
        for i, (label, cost) in enumerate(
            ...
            pi = self.severity_profile[i]
            lines.append(
                f"  {label.capitalize()} & {cost:,.0f} & {cnt:,} & {pi:.3f} \\\\"
            )
    ...
    def plot(self, output_dir: str | Path | None = None) -> dict:
        ...
        ax2.set_xticklabels(
            [
                f"{lbl}\n(${c:,.0f})"
                for lbl, c in zip(self.labels, self.cost_levels, strict=True)
            ],
            fontsize=9,
        )
        ...
        ax2.set_ylim(
            0,
            max(self.severity_profile) * 1.25 if max(self.severity_profile) > 0 else 1,
        )
        ...
        if output_dir:
            fig2.savefig(
                output_dir / "severity_profile.pdf", dpi=300, bbox_inches="tight"
            )
...
def evaluate(
    ...
    if n != len(references) or n != len(severity_annotations):
        raise ValueError(
            "predictions, references, and severity_annotations must have same length"
        )
    if n == 0:
    ...
    for p, r, s in zip(predictions, references, severity_annotations, strict=True):
        if p != r:
            if s not in label_to_idx:
                raise ValueError(
                    f"Unknown severity label '{s}'. Expected one of {labels}"
                )
